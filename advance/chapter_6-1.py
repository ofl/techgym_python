...
class Human(Player):
    ...
    def hit_or_stand(self, card_set, plotter_class):
        while True:
            if self.is_burst() or self.__stop_hitting():
                break
            else:
                self.hit_card(card_set)
                plotter_class.show_cards(self.cards)
...
class Computer(Player):
    ...
    def __stop_hitting(self):
        return self.is_blackjack() or self.min_total_number() >= 18


# バリデーションのための独立したクラスを作りたい
...
class BlackJack:
    def __init__(self, players, plotter_class=CardPlotter):
    ...
    def play_once(self):
        ...
        player.hit_card(self.card_set)
        dealer.hit_card(self.card_set)
        player.hit_card(self.card_set)
        dealer.hit_card(self.card_set)

        self.plotter_class.show_cards(player.cards)

        player.hit_or_stand(self.card_set, self.plotter_class)
    ...
    def __get_winner(self):
        ...
        if player.is_burst() and dealer.is_burst():
            return None
        elif dealer.is_burst():
            return player
        elif player.is_burst():
            return dealer

        player_total = player.number_for_judge()
        dealer_total = dealer.number_for_judge()
