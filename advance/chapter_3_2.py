...
class Game:
    ...
    def play(self):
        for i in range(self.__DEFAULT_INNING_END):
            for j in self.__both_sides_of_inning:
    ...
    def show_result(self):
        ...
        for i in self.__both_sides_of_inning:
            print(self.__get_scoreboard(i))

    def __create_inning(self, num, side):
        offensing_team = self.__teams[side]
        defensing_team = self.__teams[int(not side)]
        return Inning(num, side, offensing_team.get_hit_rate(),
    ...
    def __select_team(self, side):
        player_name = self.__SIDES_OF_INNING[side]
        selected_number = int(
    ...
    def __get_scoreboard(self, side):
        scores = self.__get_inning_scores(side)
        inning_scores = ' | '.join(list(map(str, scores)))
        team = self.__teams[side]
        total_score = sum(scores)
        return ' | '.join([team.short_name, inning_scores, str(total_score)]) + ' | '

    def __get_inning_scores(self, side):
        scores = []
    ...
    # イニングの表と裏(0または1の数字)
    @property
    def __both_sides_of_inning(self):
        return range(len(self.__SIDES_OF_INNING))


class Inning:
    def __init__(self, num, side, hit_rate, out_rate):
        self.score = 0
        self.num = num  # 回
        self.side = side  # イニングの表裏(0: 表, 1: 裏)
